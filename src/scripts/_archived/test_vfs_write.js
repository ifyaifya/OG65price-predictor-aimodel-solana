...
async function main() {
  ...
  // Write some data using VFS - using chr() since bytes() is not available
  const code = `f=open("/sol/1","w")\nf.write(chr(72)+chr(69)+chr(76)+chr(76)+chr(79))\nf.close()\n1`;
  ...
  console.log("Account data (first 32 bytes):", Buffer.from(accInfo.data).slice(0, 32).toString('hex'));
  console.log("As text:", Buffer.from(accInfo.data).slice(0, 10).toString());
}
